  ...
  return ((input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    try {
      ...
      if (origin && reqUrl.startsWith(origin)) {
        // Заголовки не трогаем: Authorization доходит до нашего роута как есть
        // (переодевание в x-sb-authorization требовалось только Serverless
        // Containers, которые перехватывали Bearer как IAM-токен).
        const rewritten = proxyBase + reqUrl.slice(origin.length);
        if (typeof input === 'string' || input instanceof URL) {
          return fetch(rewritten, init);
        }
        // Request object (rare for supabase-js): re-target the URL, keep the rest.
