...
## 1. Как применить миграции

Миграции gating'а, строго в этом порядке:

1. `supabase/migrations/20260709000000_billing_and_profiles.sql` — базовая
   (таблицы, RBAC, функции, backfill);
2. `supabase/migrations/20260709120000_admin_permanent_access.sql` — поверх
   базовой (`create or replace`): постоянный доступ для админов (см.
   раздел 2). Применяется **строго после** базовой — она переопределяет
   функции и view, созданные там.

Применяются вручную, как остальные (см. `supabase/migrations/README.md`):
открыть Supabase Studio → SQL Editor → вставить содержимое файла → Run.
Сначала на staging-инстансе, после проверки — на prod.

Обе миграции идемпотентны: повторный запуск безопасен (create table if not
...
строго: **сначала миграции, потом деплой фронта**. Обратный порядок = отказ
сервиса до применения миграций.
...
**У админов постоянный доступ** (миграция
`20260709120000_admin_permanent_access.sql`). Пользователь, у которого есть
хотя бы один `admin.*`-пермишен через любую роль (тот же критерий, что
`is_admin()`), всегда получает entitlement
`{"status":"active","kind":"admin","expires_at":null}` — независимо от
грантов в `access_grants` (истёкшие, отозванные, отсутствующие). Админ не
может «запереться снаружи» продукта, которым управляет; выдавать ему гранты
не нужно. В списке пользователей `/admin` и в `admin_user_overview` такой
пользователь показан как активный с постоянным доступом (kind `admin`,
«Доступ до» и «Осталось дней» пустые), в `admin_expiring_soon` («кому звонить
продлевать») он не попадает. Значение `kind='admin'` только вычисляется — в
`access_grants.kind` оно не хранится. Снятие последней админской роли
возвращает пользователя к обычному расчёту по грантам.

> **Внимание:** критерий — *любой* `admin.*`-пермишен, включая view-only.
> Роль `admin_readonly` (только `admin.*.view`) тоже даёт постоянный
> бесплатный доступ к продукту: назначив её, например, помощнику для
> просмотра дашборда, вы навсегда освобождаете его от оплаты — он перестаёт
> появляться в `admin_expiring_soon`, и его триал/грант больше ни на что не
> влияет, пока роль не снята. Назначайте роли с `admin.*`-пермишенами только
> тем, кому такой доступ действительно положен.
...
Админы в `admin_user_overview` показаны как `active` / `admin` с пустыми
`access_until`/`days_left`; в `admin_expiring_soon` они не попадают — их
доступ постоянный, звонить им о продлении не нужно.
...
`expires_at` используются только для текстов баннера. `kind="admin"` —
вычисляемое значение для держателей `admin.*`-пермишенов (постоянный доступ,
`expires_at` всегда `null`, баннер не показывается); в `access_grants.kind`
оно не хранится. Проверка на сервере
